        ...
        public void Start()
        {
            ...
            try
            {
                ...
                if (MinutesPassed == Interval1_archivetime || MinutesPassed == Interval2_archivetime || MinutesPassed == Interval3_archivetime || MinutesPassed == Interval4_archivetime || MinutesPassed == Interval5_archivetime || MinutesPassed == Interval6_archivetime)
                {
                    ...
                    try
                    {
                        ...
                        {
                            System.IO.File.Move(toBeProcessedfiles[0].FullName, string.Format(@"{0}\{1}", rootFolder, toBeProcessedfiles[0].Name));
                        }

                        FileInfo[] toBeAllocatedfiles = dir.GetFiles("*ToBeAllocated*", SearchOption.TopDirectoryOnly);
                        if (toBeAllocatedfiles.Count() > 0)
                        {
                            System.IO.File.Move(toBeAllocatedfiles[0].FullName, string.Format(@"{0}\{1}", rootFolder, toBeAllocatedfiles[0].Name));
                        }

                        string ordersFilePath = string.Empty;
                        FileInfo[] ordersfiles = dir.GetFiles(string.Format("*{0}*", "orders", SearchOption.TopDirectoryOnly));

                        string chargesFilePath = string.Empty;
                        FileInfo[] chargesfiles = dir.GetFiles(string.Format("*{0}*", "charges", SearchOption.TopDirectoryOnly));

                        if (chargesfiles.Count() > 0 && ordersfiles.Count() > 0)
                        {
                        ...
                        FileInfo[] catalogfiles = dir.GetFiles("*catalog*", SearchOption.TopDirectoryOnly);
                    ...
                    if (MinutesPassed == VerificationInterval1 || MinutesPassed == VerificationInterval2 || MinutesPassed == VerificationInterval3 || MinutesPassed == VerificationInterval4 || MinutesPassed == VerificationInterval5 || MinutesPassed == VerificationInterval6)
                    {
                        ...
                        try
                        {
                            ...
                            for (int i = 1; i <= numberOfBotAgents; i++)
                            {
                                ...
                                {
                                    botprocess = "Verification_DN_MultiBot",
                                    timeoutseconds = "1000",
                                    appcode = "DN005",
                                    userid = "sbsuperadmin",
                                    stepid = "",
                                ...
                                {
                                    sopName = i + "_DN_OnHold_VerificationAndDNRaising_SOP",
            ...
            catch (Exception ex)
            {
                ...
                try
                {
                    ...
                    email.SendEMail(automationTeamEmail, "ALERT : DN Automation Run Failed!! ", messagebody, "");
        ...
        public void MergeFiles(bool isEmptyFile = false)
        {
            ...
            try
            {
                            ...
                            foreach (var Encounter in FINlist)
                            {
                                ...
                                else
                                {
                                    if (!string.IsNullOrEmpty(botStatus))
                                    {
                                        ...
                                        if (botStatus == "Good Record" || botStatus == "Description not found")
                                        {

                                            if (botStatus == "Description not found")
                                            {
                ...
                if (true)
                {
                    ...
                    if (shouldDNBeRaised)
                    {
                        subject = string.Format("End of the day summary & inventory details {0}", DateTime.Now.ToString("yyyy-MM-dd HH:mm"));//ConfigurationManager.AppSettings["Email.Subject"];
                    }
                    else
                    {
                        subject = string.Format("Discern Notification Summary & Inventory Details {0}", DateTime.Now.ToString("yyyy-MM-dd HH:mm"));//ConfigurationManager.AppSettings["Email.Subject"];
                    }
